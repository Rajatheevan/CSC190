...
#Written by William Flint for CSC190
#CSC190 2018 taught by Professor Mathai
#The following function determines the 'edginess' of a position
...
def imp(board,position,player):
	...
	if (not alpha) and (board[position] == 20) or (board[position] == 21) or (board[position] == 22) or (board[position] == 23) or (board[position] == 24) or (board[position] == 25):
		...
		return False
#This function returns the piece that is making the move and the move that is to be made
		...
		if blah:
			for d in blah:
				if d==temp:
					...
					return [temp,u]
	return False
#This function is now defunct but it checks if the king is on the board
def iskng(board):
	alpha=False
	Beta = False
	for i in board:
...
#Determines if a position is under threat
def IsPositionUnderThreat(board,position,player):
	...
	if holder:
		...
		return False
#Returns the positions of each of a players pieces on a board
...
#This is the bulk of the program and returns the legal moves of the position called
...
#Determines if a position is really under threat
#This is needed as traditionally pawns cant legal move unless a piece is there,
#as such this function simulates that			
...
def PrintBoard(board):
	print board[:8],'\n',board[8:16],'\n',board[16:24],'\n',board[24:32],'\n',board[32:40],'\n',board[40:48],'\n',board[48:56],'\n',board[56:64],'\n'
	return True	
#Print board given by Professor Mathai
def printBoard(board):
   accum="---- BLACK SIDE ----\n"
   max=63
   ...
   return accum
#The Following Are the piece possible move evaulations
#denoted as 'ai' it is the desicion matrix for each piece
...
def addtoroot(root, val):
		...
		root.AddSuccessor(x)
		return True
#Uses a two level look ahead(three moves BWB or WBW) to determine the best move
...
def detree(list,player):
	...
	return rlist
#wraps nebu machine and is the main function called in the assigment
